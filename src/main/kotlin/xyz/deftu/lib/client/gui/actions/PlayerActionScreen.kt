...
class PlayerActionScreen(
    ...
    override fun initScreen(width: Int, height: Int) {
        if (MinecraftClient.getInstance().world == null) {
            MinecraftClient.getInstance().setScreen(null)
            ChatHelper.sendClientMessage(TextHelper.createTranslatableText("${DeftuLib.ID}.error.screen_requires_world", "PlayerActionScreen"))
            return
        }

        if (actions.isEmpty()) {
            MinecraftClient.getInstance().setScreen(null)
            ChatHelper.sendClientMessage(TextHelper.createTranslatableText("${DeftuLib.ID}.error.no_actions"))
            return
        }
